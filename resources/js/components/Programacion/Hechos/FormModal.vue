                            ...
                            <div class="card-body row">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group input-group-merge">
                                        <input type="text" class="form-control" v-model="hecho.nombre" >
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" for="basic-default-fullname">Peso (%)</label>
                                    <div class="input-group input-group-merge">
                                        <input type="number" class="form-control" v-model="hecho.peso">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" for="basic-default-fullname">Descripción</label>
                                    <div class="input-group input-group-merge">
                                        <input type="text" class="form-control" v-model="hecho.descripcion">
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods:{
        getPerido(){
            this.hecho.periodo_id = 1
        },
        saveProduct(){
            axios.post(!this.hecho.id ? '/hechos' : `/hechos/${this.id}`, this.hecho).then(res=>{
                console.log(res)
                if (res.data.status) {
